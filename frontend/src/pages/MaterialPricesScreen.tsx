...
import { ScrapRateItem } from '../types';
import {
  Check,
  Save,
} from 'lucide-react';
...
export const MaterialPricesScreen: React.FC = () => {
  ...
  const [rates, setRates] = useState<ScrapRateItem[]>(() => {
    ...
    return prioritizeEWasteRates(list);
  });
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      const finalized = prioritizeEWasteRates(rates);
      setRates(finalized);
      await dealerOrderService.updateScrapPrices(finalized);
      if (dealer) {
        await updateProfile({ scrapRates: finalized });
      }
      setSuccessMsg(true);
